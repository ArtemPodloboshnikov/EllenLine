...
const Cruises = (props) => {
    ...
    useEffect(()=>{
        ...
        {
            console.log(data)
            let response =  await fetch(Global.urlServer + '/api/cruises', {
                method: 'POST',
                headers: {
        ...
        async function uploadPhotos(data)
        {
            ...
            let response =  await fetch(Global.urlServer + '/api/cruisesPhotos', {
                mode: 'no-cors',
                method: 'POST',
                body: data
            });
    ...
    return (
        ...
        <form className={props.className} onSubmit={handleSubmit(handleOnSubmit)}>
            ...
            <InputText register={register({required: true})} name='titleOfCompany' className={classes.inputText} 
            classInput={classes.inputText__input} placeholder='Название компании'/>
            ...
            className={classes.select} placeholder='Тип круиза' options={['Речной', 'Морской']} type='select'/>

            <SelectEntered register={register({required: true})} name='typeOfShip'
            className={classes.select} placeholder='Тип круиза' options={['Лайнер', 'Теплоход']} type='select'/>

            <InputText register={register({required: true})} name='typeOfRoom' className={classes.inputText + ' ' + classes.inputText_width50} 
            classInput={classes.inputText__input} placeholder='Тип каюты'/>
            ...
            className={classes.inputNumber} placeholder='Кл. кают' min='1'/>

            <InputNumber classWrap={classes.inputNumber_wrap + ' ' + classes.number_top} register={register({required: true})} name='duration' 
            className={classes.inputNumber} placeholder='Продолжительность (дни)' min='1'/>
